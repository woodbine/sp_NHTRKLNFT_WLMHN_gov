...
entity_id = "NHTRKLNFT_WLMHN_gov"
url = "http://www.wlmht.nhs.uk/about-wlmht/how-we-are-run/finance-and-capital/"
errors = 0
data = []
...
for block in blocks:
    if '.csv' in block['href'] or '.xls' in block['href'] or '.xlsx' in block['href'] or '.pdf' in block['href']:
        if 'http' not in block['href']:
            link = 'http://www.wlmht.nhs.uk'+block['href'].strip()
        else:
            link = block['href'].strip()
